...
;
;
pro fly_tirs_for_lon, image_filename, ghost_map_filename, $
...
n_positions = n_elements( positions[0,*] )
  ...
  for i = 00, n_positions-1 do  begin
    ...
    image_array_display[*,*, i ] = image_lon_2D
...
for band = 0,n_positions-1 do begin  
    ...
    line_pos = sorted_indices[ 1, band ]
      ...
      ground_track_values[ band ] = image_lon_2D[ground_track_sample, ground_track_line]
 
      goes_subsection = image_lon_2D[ground_track_sample-102:ground_track_sample+102, ground_track_line-102:ground_track_line+102]
...
wset,0
  ...
  envi_write_envi_file, radiance_array,out_name='LON-'+output_file_basename+'-unsampled'+'.img',bnames=["Vector Hits","Sum Weights","Sum^2 Weights","Ghost Weights",band_names], map_info=map_info

  envi_write_envi_file, ghost_radiance_array,out_name='LON-'+output_file_basename+'-'+'sampled'+'.img',bnames=["Vector Hits","Sum Weights","Sum^2 Weights","Ghost Weights",band_names], map_info=map_info
...
for i = 0, n_positions+3 do begin
  ...
  envi_write_envi_file, flicker_array,out_name='LON-'+output_file_basename+'-'+'flicker'+'.img'
